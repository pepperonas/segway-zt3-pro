...
| Auto-Apply des Boot-Profils feuerte sofort beim Connect, parallel zum Pairing-Init | 1.5 s Delay + Toggle in Settings (Default OFF) |
| Pairing-encrypt() returnte null wenn Session nicht versiegelt | Plaintext-Fallback mit Frame-Wrap |

### Nächster Schritt

`5A A5`-Classic-Stack implementieren (siehe `app/README.md` → Open Items § Classic-Path).
